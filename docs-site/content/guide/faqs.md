...
- **Hosted MCP Server:** We offer an official [Hosted MCP Server on Typesense Cloud](./typesense-cloud/mcp-server.md) that you can connect with your AI agents like Claude Code, Codex, Cursor, etc to interact with your clusters using your agents. This MCP Server is deeply integrated with the RBAC permission system in Typesense Cloud, so if you can have different members of your team with varying levels of access, they'll only get access to the subset of capabilities you've defined via their agent, just like in the web UI.
