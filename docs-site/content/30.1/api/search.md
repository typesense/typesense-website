...
The highest-weighted field is the **first** field listed in `query_by`. The one exception is when `query_by_weights` gives a higher weight to a field further down the list, in which case that field's settings govern query parsing.
