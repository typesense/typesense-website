...
- ⚠️ **Overrides** (aka Curation Rules) are no longer nested under Collection. Similar to synonym sets, we now have a top-level resource called a "**Curation Set**" that can be attached to one or more collection, or can be dynamically sent as a search parameter.  The [`override_tags`](https://typesense.org/docs/29.0/api/search.html#ranking-and-sorting-parameters) parameter has been renamed to `curation_tags`. Existing overrides will be auto-migrated to curation sets automatically on upgrading. [(Docs)](https://typesense.org/docs/30.0/api/curation.html)
