import { getCopyLanguageByLabel, getCopyLanguageBySlug, normalizeCopyLanguages } from './copyLanguages'
import type { CopyLanguageLabel } from './copyLanguages'
...
function readRawLanguagesFromLocation(): string | null {
...
function readPreferredCopyLanguagesFromUrl(): CopyLanguageLabel[] | null {
  try {
    ...
    const labels = raw
      ...
      .map(slug => getCopyLanguageBySlug(slug)?.label)
      .filter((label): label is CopyLanguageLabel => Boolean(label))
...
function readPreferredCopyLanguagesFromStorage(): CopyLanguageLabel[] {
...
function stripLanguagesFromHash(url: URL): void {
  if (!url.hash) {
    return
  }
...
function syncPreferredCopyLanguagesToUrl(languages: unknown): void {
  ...
  try {
    ...
    const slugs = normalizeCopyLanguages(languages)
      .map(label => getCopyLanguageByLabel(label)?.slug)
      .filter(Boolean)
...
function readPreferredCopyLanguages(): CopyLanguageLabel[] {
...
function writePreferredCopyLanguages(languages: unknown): CopyLanguageLabel[] {
