---
title: "Typesense Search API Parameters"
description: "Use the Typesense Search API with query parameters for full-text search, filters, facets, sorting, grouping, typo tolerance, and pagination."
sidebarDepth: 2
sitemap:
  priority: 0.7
---
...
| <span id="stopwords">stopwords</span>            | no       | A comma separated list of stopword sets to be dropped from the search query while searching. For more information on stopword sets, you can refer to the <RouterLink :to="`/${$site.themeConfig.typesenseLatestVersion}/api/stopwords.html`">Stopwords documentation</RouterLink>.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
:::warning Query tokenization follows the highest-weighted query_by field
The search query `q` is tokenized **once**, using the tokenization settings of the **highest-weighted** field in `query_by`. That covers `locale`, `token_separators`, `symbols_to_index` and stemming (`stem`). Those same query tokens are then reused to search every other `query_by` field, and the other fields' own tokenization settings are **not** applied when parsing the query. Synonym expansion also uses this field.

The highest-weighted field is the **first** field listed in `query_by`. The one exception is when [`query_by_weights`](#query_by_weights) gives a higher weight to a field further down the list, in which case that field's settings govern query parsing.

This applies only to **query parsing**. Each field is still indexed, faceted and highlighted using its own tokenization settings. If two queried fields differ in their `token_separators`, `symbols_to_index` or `locale`, make sure the highest-weighted field's settings are compatible with how the other fields were indexed. Otherwise a query token may not line up with how another field was tokenized at index time.
:::
...
| Parameter | Required | Description                                                                                                                                                                                                                                                                                                                                                  |
|:----------|:---------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <span id="use_cache">use_cache</span> | no       | Enable server side caching of search query results. By default, caching is disabled.<br><br>When using <RouterLink :to="`/${$site.themeConfig.typesenseLatestVersion}/api/federated-multi-search.html`">multi_search</RouterLink>, this parameter needs to be specified as a URL query parameter, and NOT in the POST request body. <br><br>Default: `false` |
