...
| `geopolygon` | Geographic polygon defined by an array of coordinates specified as `[lat1, lng1, lat2, lng2, ...]`. Latitude/longitude pairs must be in counter-clockwise (CCW) or clockwise (CW) order. Read more <RouterLink :to="`/${$site.themeConfig.typesenseLatestVersion}/api/geosearch.html#geographic-polygons`">here</RouterLink>. |
